...
def main() -> None:
    ...
    # ---- 1. occurrence -------------------------------------------------
    # The bundled series is ONI. For another hazard, load your own index here.
    series = load_sample("oni")
    ...
    # Fit on labelled rows, but PREDICT from the most recent features available.
    # The last `horizon` periods have no label yet -- and that is precisely the
    # window in which an event may be developing. Predicting from the last
    # LABELLED row instead silently hands you a stale vintage. That is not a
    # nicety: the project this library came from presented a March-vintage
    # probability in August, while the index had already tripled.
    ...
    # ---- 3. Monte Carlo ------------------------------------------------
    # Probabilities are explicit and required. Passing the observed prior for
    # severity is a choice: it is defensible and transparent, but it means the
    # simulation carries no forward-looking severity information.
    mc = MonteCarloEngine(
        spec,
